...

{
	...
	public $heading="Article Management";
...

	{
		...
		if (!Yii::app()->user->hasState("admin")){
			...
			$cms=  Articles::model()->findAll($cri);
...

	{
		...
		if (!empty($_POST)){
                ...
                $check=Articles::model()->exists($cri);



                if ($check){

	                $this->redirect(Yii::app()->createUrl("octa/articles/addarticle",array("msg"=>"2")));
...

                {
					...
					if(!empty($post_image)){
						...
						$featureimg = Articles::model()->findByPk($lastid);
                	...
                	$this->redirect(Yii::app()->createUrl("octa/articles/addarticle",array("msg"=>"1")));
					...
					$this->redirect(Yii::app()->createUrl("octa/articles/addarticle",array("msg"=>"0")));
...

	{
		...
		if (!empty($_POST)){
...

			{
				...
				$this->redirect(Yii::app()->createUrl("octa/articles/edit",array("id"=>$id,"msg"=>$msg)));
...

	{
		...
		Articles::model()->deleteByPk($id);

		$this->redirect(Yii::app()->createUrl("octa/articles"));
